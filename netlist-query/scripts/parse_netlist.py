...
==================================
解析 Cadence Allegro / OrCAD 导出的 pstxprt.dat, pstxnet.dat, pstchip.dat
三个网表文件，生成结构化 JSON，方便 AI 查询器件信息和网络连接。

用法:
    python3 parse_netlist.py <allegro_dir> [--name <设计名>] [--version <版本号>]

输出目录: ~/.cache/skills/netlist-query/<设计名>/

allegro_dir: 包含 pstchip.dat / pstxprt.dat / pstxnet.dat 的目录
设计名:     默认取 allegro_dir 的目录名
版本号:     默认从 pstxprt.dat 的 POST_TIME 推断，或用当前日期
"""
    ...
    for block in blocks:
        ...
        if pin_section:
            for pm in re.finditer(
                ...
                pins[pm.group(1)] = {"number": pm.group(2).strip('()'), "use": pm.group(3)}
...
def parse_pstxprt(filepath: str) -> tuple:
    """解析 pstxprt.dat — 元件实例列表（RefDes → 器件类型）
    
    返回: (components_dict, design_name, post_time)
    """
    components = {}
    with open(filepath, 'r', encoding='utf-8', errors='replace') as f:
        text = f.read()
...
def classify_component(refdes: str, primitive: str = "", value: str = "") -> str:
    ...
    categories = {
        ...
        "Q": "transistor", "U": "ic", "J": "connector", "CON": "connector",
        "TP": "testpoint", "F": "fuse", "FB": "ferrite_bead",
        "Y": "crystal", "SW": "switch", "LED": "led",
    }
...
def build_json(allegro_dir: str, design_name_override: str = None,
    ...
    raw_components, auto_design_name, post_time, pstwriter_info = parse_pstxprt(xprt_file)
    print("Parsing pstxnet.dat ...")
    nets = parse_pstxnet(xnet_file)

    design_name = design_name_override or auto_design_name or os.path.basename(allegro_dir)
    ...
    for refdes, comp in raw_components.items():
        ...
        components[refdes] = {
            "refdes": refdes, "part_name": part_name, "value": value,
            "footprint": jedec, "category": category,
        ...
        if prim.get("pins"):
            for pin_name, pin_info in prim["pins"].items():
                components[refdes]["pins"][pin_info["number"]] = {
                    "name": pin_name, "number": pin_info["number"], "use": pin_info["use"],
                }
    ...
    for refdes, comp in sorted(components.items()):
        cat_index[comp["category"]].append({
            "refdes": refdes, "part_name": comp["part_name"],
    ...
    for cat, items in sorted(cat_index.items()):
        ...
        for part_name, refdes_list in sorted(by_part.items()):
            ...
            parts_list.append({
                "part_name": part_name, "value": sample["value"],
                "footprint": sample["footprint"], "count": len(refdes_list),
                "refdes_list": sorted(refdes_list,
                    key=lambda r: (re.match(r'^[A-Z]+', r).group() if re.match(r'^[A-Z]+', r) else '',
                                   int(re.search(r'\d+', r).group()) if re.search(r'\d+', r) else 0)),
            })
        category_summary[cat] = {
            "total": len(items), "unique_parts": len(by_part),
    ...
    passive_count = sum(1 for c in components.values()
    ...
    return {
        "_meta": {
            ...
            "stats": {
                "total_components": len(components), "total_nets": len(nets),
                "ics": ic_count, "connectors": conn_count, "passives": passive_count,
            },
        },
...
def main():
    ...
    parser.add_argument("allegro_dir",
                        help="包含 pstchip.dat / pstxprt.dat / pstxnet.dat 的目录")
    parser.add_argument("--name", default=None,
                        help="设计名 (默认: 从 ROOT_DRAWING 或目录名推断)")
    parser.add_argument("--version", default=None,
                        help="版本号 (默认: 从 POST_TIME 推断或用当前日期)")
    ...
    if not memory_file.is_file():
        memory_file.write_text(
            f"# {design_name} - 网表查询缓存\n\n"
            f"> 本文件记录已查询过的电路连接信息，用于后续快速定位。\n\n"
            "（暂无记录）\n",
            encoding="utf-8",
        )
    ...
    print(f"   Raw backup: {out_dir / 'raw/'} ({len(copied)} files: {', '.join(copied)})")
